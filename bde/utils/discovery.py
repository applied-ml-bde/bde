"""Discover available sklearn objects.

The :mod:`bde.utils.discovery` module includes utilities to discover
objects (i.e. estimators, displays, functions) from the `bde` package.
"""
...
def all_displays():
    """Get a list of all displays from `bde`.
    ...
    Examples
    --------
    >>> from bde.utils.discovery import all_displays
...
def _is_checked_function(item):
    ...
    mod = item.__module__
    if not mod.startswith("bde.") or mod.endswith("estimator_checks"):
        return False
...
def all_functions():
    """Get a list of all functions from `bde`.
