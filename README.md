...
1. Define a fully connected neural network structure where each output value corresponds to a parametrization of a distribution. 
   ...
   mean $\mu$ and the logarithm of the standard deviation $\log(\sigma)$, and the output layer for a network with N predicted values should look as follows: $(\mu_1, \mu_2, ..., \mu_N, \log(\sigma_1), \log(\sigma_2), ..., \log(\sigma_N))$.
